...
DECLARE
  ...
  uid uuid := auth.uid();
  v_new_purity numeric;
  v_new_da numeric;
  v_new_gold_stock numeric;
  v_new_da_stock numeric;
...
BEGIN
  ...
  v_new_purity := cli.purity_balance;
  v_new_da := cli.da_balance;
  v_new_gold_stock := stk.pure_gold_stock;
  v_new_da_stock := stk.da_stock;

  IF tx.direction = 'receiving' AND tx.transaction_type = 'da' THEN
    v_new_da := cli.da_balance + tx.da_amount;
    v_new_da_stock := stk.da_stock + tx.da_amount;
  ...
  ELSIF tx.direction = 'receiving' AND tx.transaction_type = 'gold' THEN
    v_new_purity := cli.purity_balance + tx.total_pure_weight;
    v_new_gold_stock := stk.pure_gold_stock + tx.total_pure_weight;
    v_new_da := cli.da_balance - tx.total_refining_fee;
  ...
  UPDATE public.refinery_clients
     SET purity_balance = v_new_purity, da_balance = v_new_da
   WHERE id = cli.id;

  UPDATE public.refinery_stock
     SET pure_gold_stock = v_new_gold_stock, da_stock = v_new_da_stock, updated_at = now()
  ...
  INSERT INTO public.refinery_stock_movements(
    ...
    gold_change, da_change,
    stk.pure_gold_stock, v_new_gold_stock, stk.da_stock, v_new_da_stock,
    uid
  );
  ...
  UPDATE public.refinery_transactions
     SET status = 'settled',
         ...
         new_purity_balance = v_new_purity,
         previous_da_balance = cli.da_balance,
         new_da_balance = v_new_da,
         previous_gold_stock = stk.pure_gold_stock,
         new_gold_stock = v_new_gold_stock,
         previous_da_stock = stk.da_stock,
         new_da_stock = v_new_da_stock
   WHERE id = tx.id
   RETURNING * INTO tx;
